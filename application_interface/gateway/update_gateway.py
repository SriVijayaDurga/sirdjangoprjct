    ...
    if payload != 0:
        try:
            if payload['permission']['update']:
                ...
                update_tuple = (str(request_body[REQ_GATEWAY_MAC_ID]), request_body[REQ_GATEWAY_COMMISSION],
                                str(request_body[REQ_GATEWAY_USERID]), gateway_id, entity_id)
                # Parameterized update query
                data, err = django_parameterized_query_update(update_sql=updateGateway_query,
