    ...
    if payload != 0:
        try:
            if payload['permission']['create']:
                ...
                insert_tuple = (str(request_body[REQ_GATEWAY_MAC_ID]), str(request_body[REQ_GATEWAY_ID]),
                                request_body[REQ_GATEWAY_COMMISSION], str(request_body[REQ_GATEWAY_USERID]),
                                entity_id)
                # Parameterized insert query
                data, err = django_parameterized_query_insert(insert_sql=addNewGateway_query,
