    ...
    if payload != 0:
        try:
            if payload['permission']['update']:
                ...
                update_tuple = (str(request_body[REQ_TICKET_CAUSE]), str(request_body[REQ_TICKET_REMARK]),
                                request_body[REQ_TICKET_SEVERITY], str(request_body[REQ_TICKET_STATUS]),
                                userid, request_body[REQ_TICKET_REPORTED_DATE], request_body[REQ_TICKET_CLOSED_DATE],
                                request_body[REQ_TICKET_MACHINEID], ticketid, entity_id)
                # Parameterized update query
                data, err = django_parameterized_query_update(update_sql=updateTicket_query,
